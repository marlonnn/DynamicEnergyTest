...
namespace DynamicEnergyTest.DBClass
{
    ...
    public class SQLiteOperate : DBOperate
    {
        ...
        public override bool Connect(bool exclusive)
        {
            ...
            //if (NovoCyte.Program.EnableSQLiteSyncFull)
            //{
            //    this.conn = new SQLiteConnection(connsb.ToString(), true);

            //    if (this.Open())
            //    {
            //        if (TryExecuteNonQuery("PRAGMA synchronous = full;", null) != -1)
        ...
        /// <summary>
        /// for sqlite connect, if network is disconnect and reconnect, there will be IoErr exception,
        /// need to close and re-open the connection
        /// </summary>
        /// <returns>true if successful</returns>
        protected override bool RecoverFromConnectionLost(Exception e)
        {
            var se = e as SQLiteException;

            if (se != null)
            {
                System.Diagnostics.Trace.WriteLine(se.ResultCode);
            }
            if (se != null && (se.ResultCode == SQLiteErrorCode.IoErr || se.ResultCode == SQLiteErrorCode.CantOpen))
            {
                Close(false);
                return Open();
            }
