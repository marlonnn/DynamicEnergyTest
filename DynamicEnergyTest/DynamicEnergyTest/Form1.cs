        ...
        public Form1()
        {
            ...
            try
            {
                ...
                using (var writer = new FileStream("ProcessItems.xml", FileMode.Create))
                {
                    ...
                    list.Add(new ProcessItem { Index = 1, TestTitle = "条码测试", TestContent = "扫描单板上条码,与PCBA主板上实际烧录的内容是否一致。", FunCode = 0x00710012 });
                    list.Add(new ProcessItem { Index = 2, TestTitle = "版本测试", TestContent = "与设置预设版本一致进入下一环节测试；\n 不一致则退出测试模式；", FunCode = 0x00710001 });
                ...
                using (var reader = new StreamReader(System.Environment.CurrentDirectory + "\\ProcessItems.xml"))
                {
                    XmlSerializer deserializer = new XmlSerializer(typeof(List<ProcessItem>),
                        new XmlRootAttribute("ProcessCollection"));
                    var ProcessItems = (List<ProcessItem>)deserializer.Deserialize(reader);
                }
            }
            catch (Exception ex)
            {
